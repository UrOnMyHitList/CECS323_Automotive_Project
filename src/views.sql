...
SELECT CUSTOMERID, (((YEAR(NOW())-(SELECT YEARJOINED FROM CUSTOMER WHERE PREMIER.CUSTOMERID = CUSTOMER.CUSTOMERID))*12)*MONTHLYFEE AS MONTHLYFEE,
(SELECT SUM(TOTALCOST) FROM SERVICEVISIT SV WHERE SV.CUSTOMERID = PREMIER.CUSTOMERID) AS SERVICEVISITCOST,
(((YEAR(NOW())-(SELECT YEARJOINED FROM CUSTOMER WHERE PREMIER.CUSTOMERID = CUSTOMER.CUSTOMERID))*12)*MONTHLYFEE
